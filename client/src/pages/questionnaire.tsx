...
export default function Questionnaire() {
  ...
  const archetypeMutation = useMutation({
    mutationFn: (responses: Record<string, string>) => 
      apiRequest('/api/archetype', 'POST', { responses }),
    onSuccess: (data) => {
      setIsComplete(true);
  ...
  if (isComplete) {
    return (
      <div className="min-h-screen p-8 bg-gradient-to-b from-space to-void texture-organic flex items-center justify-center">
        <Card className="card-legendary bg-gradient-to-br from-void to-purple-deep border-holo-gold max-w-2xl w-full">
          <CardHeader className="text-center">
            <div className="w-16 h-16 bg-gradient-to-br from-electric-green to-neon-cyan rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle className="text-space" size={32} />
            </div>
            <CardTitle className="text-3xl font-cyber text-holo-gold mb-2">
              Welcome to Village-One!
            </CardTitle>
          </CardHeader>
          <CardContent className="text-center space-y-6">
            <p className="text-lg text-gray-300">
              Your archetype assessment is complete. You've been identified as a:
            </p>
            
            <div className="p-6 rounded-lg bg-gradient-to-r from-electric-green/20 to-neon-cyan/20 border border-electric-green">
              <h3 className="text-2xl font-bold text-electric-green mb-2" data-testid="text-determined-archetype">
                {archetypeMutation.data?.archetype || 'Builder'}
              </h3>
              <p className="text-gray-300">
                This archetype represents your natural strengths and preferred ways of contributing to the village community.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <Button
                ...
                className="w-full bg-gradient-to-r from-neon-cyan to-electric-green text-space font-bold py-3 rounded-lg"
              ...
              <Button
                ...
                variant="outline"
                className="w-full border-holo-gold text-holo-gold hover:bg-holo-gold hover:text-space"
    ...
    <div className="min-h-screen p-8 bg-gradient-to-b from-space to-void texture-organic">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-cyber font-bold text-neon-cyan mb-4" data-testid="text-questionnaire-title">
            Discover Your Village Archetype
          </h1>
          <p className="text-xl text-gray-400">
            Answer these questions to find your ideal role in Village-One
          </p>
        </div>
        ...
        <div className="mb-8">
          ...
          <Progress value={progress} className="h-2 bg-space" />
        </div>
        ...
        <Card className="card-epic bg-gradient-to-br from-void to-purple-deep border-electric-green mb-8">
          <CardHeader>
            <div className="flex items-center space-x-3 mb-4">
              <div className="w-10 h-10 bg-gradient-to-br from-electric-green to-neon-cyan rounded-lg flex items-center justify-center">
                <Brain className="text-space" size={20} />
              </div>
              <CardTitle className="text-2xl font-cyber text-electric-green">
                Archetype Assessment
              </CardTitle>
            </div>
            <h2 className="text-xl text-white leading-relaxed" data-testid={`text-question-${currentQuestion}`}>
          ...
          <CardContent className="space-y-4">
            ...
            >
              {QUESTIONS[currentQuestion]?.options.map((option, index) => (
                <div key={option.value} className="flex items-center space-x-3 p-4 rounded-lg border border-purple-deep hover:border-neon-cyan transition-colors cursor-pointer">
                  <RadioGroupItem 
                    value={option.value} 
                    id={option.value}
        ...
        <div className="flex justify-between">
          <Button
            ...
            variant="outline"
            className="border-purple-deep text-gray-300 disabled:opacity-50"
          ...
          <Button
            onClick={handleNext}
            disabled={!currentAnswer || archetypeMutation.isPending}
            className="bg-gradient-to-r from-neon-cyan to-electric-green text-space font-semibold"
